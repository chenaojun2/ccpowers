...
2. **提供可视化伴侣**（如果主题涉及视觉问题）— 这是一条独立消息，不要与澄清问题合并。参见下方可视化伴侣章节。
3. **提出澄清问题** — 逐个提问，了解目的/约束/成功标准
4. **提出 2-3 种方案** — 包含权衡和你的推荐
5. **展示设计** — 按各部分复杂度分段展示，每段后获取用户确认
6. **撰写设计文档** — 保存到 `docs/specs/YYYY-MM-DD-<主题>-design.md` 并提交
7. **规格自审** — 快速检查占位符、矛盾、模糊之处、范围
...
基于浏览器的伴侣工具，用于在头脑风暴过程中展示原型、图表和视觉选项。它是一个工具——不是一种模式。接受可视化伴侣意味着它可用于受益于视觉展示的问题；并**不**意味着每个问题都走浏览器。

**提供可视化伴侣：** 当你预期即将到来的问题涉及视觉内容（原型、布局、图表）时，征求一次同意：
> "我们接下来讨论的一些内容，如果能在浏览器里直接展示可能更容易理解。我可以在讨论过程中展示原型、图表、对比方案和其他可视化内容。这个功能还比较新，会消耗较多 token。要试试吗？（需要打开一个本地 URL）"

**这个提议必须是一条独立消息。** 不要与澄清问题、上下文摘要或其他内容合并。消息中只包含上述提议，不包含其他任何内容。等待用户回复后再继续。如果他们拒绝，继续纯文本头脑风暴。

**逐问题决策：** 即使用户接受了，也要**对每个问题**独立判断是否使用浏览器。判断标准：**用户看到它是否比读到它更好理解？**

- **使用浏览器** 展示本身就是视觉性的内容 — 原型、线框图、布局对比、架构图、并排视觉设计
- **使用终端** 展示文本性内容 — 需求问题、概念选择、权衡列表、A/B/C/D 文字选项、范围决策

关于 UI 话题的问题不等于视觉问题。"在这个上下文中个性化是什么意思？"是概念问题——使用终端。"这两种向导布局哪个更好？"是视觉问题——使用浏览器。

如果用户同意使用可视化伴侣，在继续之前先阅读详细指南：
